...
function getConfigPath(env = process.env) {
  ...
  if (!home) throw new Error('Cannot find home directory for h5hub config');
...
async function readConfig(env = process.env) {
  try {
    ...
    if (error.code === 'ENOENT') return readLegacyConfig(env);
    if (error instanceof SyntaxError) {
      throw new Error('Invalid h5hub config JSON. Run `node scripts/publish-html.mjs login <invite-code>` again.');
    }
    throw error;
  }
}

async function readLegacyConfig(env = process.env) {
  const home = env.H5PUBLISH_CONFIG_HOME || env.HOME || env.USERPROFILE;
  if (!home) return {};
  try {
    const text = await fs.readFile(path.join(home, LEGACY_CONFIG_DIRNAME, CONFIG_FILENAME), 'utf8');
    const config = JSON.parse(text);
    return config && typeof config === 'object' ? config : {};
  } catch (error) {
    if (error.code === 'ENOENT') return {};
    if (error instanceof SyntaxError) {
      throw new Error('Invalid legacy h5publish config JSON. Run `node scripts/publish-html.mjs login <invite-code>` again.');
    }
    throw error;
  }
}
...
function normalizeWorkerUrl(value) {
  try {
    ...
    throw new Error('H5HUB_WORKER_URL must be a valid http(s) URL');
  }
}
...
try {
  ...
  if (options.command === 'login') {
    ...
    process.stdout.write(`h5hub login saved\nConfig: ${configPath}\nURL: ${options.workerUrl}\n`);
    process.exit(0);
  }
